...
namespace Assignment_2
{
    public partial class StockList
    {
        ...
        public void SortByValue()
        {
            ...
            {
                for (nextNode = this.head; nextNode.Next != null; nextNode = nextNode.Next)
                {
                    if (nextNode.StockHolding.Holdings < nextNode.Next.StockHolding.Holdings)
                    {
                        //Swapping the nodes if the next current node is greater than the next node

                        nextNode = Swap(nextNode.StockHolding);
                        //StockNode temp = nextNode.Next;
        ...
        public void SortByName()
        {
            ...
            for (int i = 0; i < this.Length(); i++)
            {
                for (nextNode = this.head; nextNode.Next != null; nextNode = nextNode.Next)
                {
                    if (nextNode.StockHolding.Name.CompareTo(nextNode.Next.StockHolding.Name) > 0)
                    {
                        //Swapping the nodes if the next current node is greater than the next node
